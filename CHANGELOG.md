...
- **Double-click setup.** Double-clicking `bg3-bridge.exe` (or running it with no arguments in an interactive console) now installs the companion mod and prints the MCP config block to paste into your agent, then holds the window open — deliberately minimal, since the agent handles connecting and testing once the config is in. Also available as `bg3-bridge setup`. When an MCP client launches the exe with no arguments over a pipe it still starts the server exactly as before (dispatch branches on `stdin.isTTY`); `bg3-bridge serve` forces server mode. `install` gained a `performInstall` core that throws rather than exits, so setup can report a failure (e.g. the game still open) and pause instead of the window vanishing.
